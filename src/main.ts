...
function getEnvConfig() {
  return {
    ...
    systemMessage: import.meta.env.VITE_AZURE_OPENAI_SYSTEM_MESSAGE || '',
    temperature: parseFloat(import.meta.env.VITE_AZURE_OPENAI_TEMPERATURE || '0.8'),
    voice: import.meta.env.VITE_AZURE_OPENAI_VOICE as Voice || '',
    useAzureOpenAI: import.meta.env.VITE_USE_AZURE_OPENAI === 'true'
  };
}
...
formEndpointField.value = config.endpoint;
formApiKeyField.value = config.apiKey;
formDeploymentOrModelField.value = config.deployment;
formSessionInstructionsField.value = config.systemMessage;
formTemperatureField.value = config.temperature.toString();
formVoiceSelection.value = config.voice;
formAzureToggle.checked = config.useAzureOpenAI;
